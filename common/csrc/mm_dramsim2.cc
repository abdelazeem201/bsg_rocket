  ...
  bool b_ready)
{
  ...
  uint64_t mask_ar_size = 0x7 & ar_size; // 3-bits in hw
  uint64_t mask_ar_len = 0xff & ar_len;  // 8-bits in hw

  uint64_t mask_aw_size = 0x7 & aw_size; // 3-bits in hw
  uint64_t mask_aw_len = 0xff & aw_len;  // 8-bits in hw

  if (ar_fire) {
    rreq[ar_addr] = mm_req_t(ar_id, 1 << mask_ar_size, mask_ar_len + 1, ar_addr);
    mem->addTransaction(false, ar_addr);
  }

  if (aw_fire) {
    store_addr = aw_addr;
    store_size = 1 << mask_aw_size;
    store_id = aw_id;
    store_count = mask_aw_len + 1;
    store_inflight = true;
  }
