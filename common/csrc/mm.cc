  ...
  bool b_ready)
{
  ...
  if (ar_fire) {
    uint64_t word_size = (1 << (0x3 & ar_size));
    for (int i = 0; i <= (0xff & ar_len); i++) {
      auto dat = read(ar_addr + i * word_size, word_size);
      rresp.push(mm_rresp_t(ar_id, dat, i == (0xff & ar_len)));
    }
  }
...
void load_mem(void** mems, const char* fn, int line_size, int id)
{
  char* m;
  ssize_t start = 0;
  std::ifstream in(fn);
  ...
  while (std::getline(in, line))
  {
    ...
    for (ssize_t i = line.length()-2, j = 0; i >= 0; i -= 2, j++) {
      ...
      m = (char *) mems[id];
      addr = (addr / line_size) * line_size + (addr % line_size);
      m[addr] = data;
    }
    start += line.length()/2;
  }
