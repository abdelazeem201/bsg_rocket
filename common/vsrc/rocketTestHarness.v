...
`endif
  ...
  begin
    ...
    begin
`ifdef DEBUG
      // Dump waveform to help debug X's sent to C++
      if ( |htif_out_valid === 1'bx ) $vcdplusflush;
`endif
      htif_tick
      (
...
`endif
  ...
  begin
    ...
    begin
      ...
      $value$plusargs("loadmem=%s", loadmem);
      if (loadmem)
        $fwrite(32'h80000002,"*** PASS %s\n", loadmem);
      $display("\n\nSimulated %d active cycles out of %d total cycles.\n\n", trace_count, total_cycle_count);
      htif_fini(1'b0);
    end
  end
...
`ifdef GATE_LEVEL
    if (verbose)
    begin
      $fdisplay(stderr, "C: %10d", total_cycle_count-1);
    end
`endif
  end
